...
const AddContacts = (props) => {
    ...
    if (data.length === 0) {
      ...
      for (const c of props.route.params.contacts) {
        ...
        for (const p of c.phoneNumbers) {
          // console.log(p.id)
          if (!num.includes(p.number)) num.push(p.number);
        }
        if (
          !name.includes(
      ...
      const sortContacts = [].slice.call(contact).sort((a, b) => {
  ...
  const add = () => {
    ...
    axios
      .put(`${ENV.apiUrl}/user/addemergencycontacts`, {
        userId: props.route.params.userId,
        emergencyContacts: emergencyContacts,
      })
      .then((response) => {
        // console.log(response.data.helpers);
        if (response.data.userid) console.log("Contacts Registered!!!");
      })
      .catch((error) => {
        console.log(error);
      });
