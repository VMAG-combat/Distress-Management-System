...
const AddFriend = (props) => {
    ...
    const loadFindPeople = useCallback(async () => {
        ...
        // console.log(props.route.params.route.params.userId)
        try {
            // const result = await dispatch(usersActions.fetchFindPeopleUsers());
            console.log("hi")
            axios({
                method: "GET",
                url: `${ENV.apiUrl}/user/getfollowusers/`+props.route.params.route.params.userId,      
              }).then(res => {
    ...
    return (
        <Container style={{ backgroundColor: '#fff' }} >
            <Header style={{ backgroundColor: "blue" }} searchBar rounded>
                <Item>
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <UserList item={item} followHandler={followHandlerForData} userid={props.route.params.route.params.userId} />
                )} 
            />
        </Container>
        // <></>
