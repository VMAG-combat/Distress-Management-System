...
class BachaoButton extends React.Component {
  ...
  registerIncident = async () => {
    try {
      ...
      deviceStorage.getId().then((userId) => {
        ...
        //   + ' ' + hours + ':' + min + ':' + sec
        axios
          .post(`${ENV.apiUrl}/incident/registerIncident`, {
            userid: userId,
          ...
          .then((response) => {
            ...
            var temp = [];
            for (const h of response.data.helpers) temp.push(h.phone);

            var messsage =
              "I'm is Distress. Please Help me. \nMy Location Coordinates are given below: \nLatitude: " +
              coord[0] +
              "\nLongitude: " +
              coord[1] +
            ...
            deviceStorage.saveKey(
              "helpers",
              JSON.stringify(response.data.helpers)
            );

            console.log("Incident Registered!!!", response.data);
  ...
  render() {
    ...
    const galleryX = this.mode.interpolate({
      inputRange: [0, 1],
      outputRange: [0.01244 * height, -0.046 * height],
    });
          ...
          <TouchableHighlight onPress={this.handdlePress} underlayColor={0}>
            <Animated.View
              style={{
                ...
                height: 0.1094 * height,
                width: 0.1094 * height,
                borderRadius: 0.08717 * height,
                justifyContent: "center",
                alignItems: "center",
                shadowColor: "black",
                shadowOffset: {
                  width: 0.1244 * height,
                  height: 0.1244 * height,
                },
                shadowOpacity: 1,
              }}
            >
