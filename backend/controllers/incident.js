...
const notify = require("../crud/notify.js");
      ...
      if (user.fcmToken) {
        notify({
          ...
          payload: {
            notification: {
              title: "Help Needed!!",
              body: "Someone needs help, go to their rescue now.",
            },
            data: {
              incidentId: id,
            },
...
exports.updateIncident = async (req, res) => {
  ...
  try {
    ...
    for (const email of newIncident.helpers) {
      var helper = await get({
        collection: "User",
        by: "where",
        where: [{ parameter: "email", comparison: "==", value: email }],
      });
...
exports.getAllActiveIncidents = async (req, res) => {
  try {
    var incidents = await get({
      collection: "Incident",
      by: "where",
      where: [{ parameter: "status", comparison: "==", value: "active" }],
    });
